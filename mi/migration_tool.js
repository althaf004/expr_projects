...
// Fetch Schema of sourcedb
    const sourceSchemaQuery = `
      SELECT schema_name
      FROM information_schema.schemata
      WHERE schema_name NOT LIKE 'pg_%' AND schema_name != 'information_schema';
    `;
// Fetch Schema of targetdb
...
//write the missing schemas to file
    ...
    const constraintsScripts = [];
    const constraintScripts = new Set();

    for (const schema of sourceSchemas) { // for each schemas create the missing Tables
      const tableQuery = `
        SELECT table_name
      ...
      for (const sourceTable of tables) {
        ...
        if (!targetTable) {
          ...
          `;
// SQL query to fetch all constraints except CHECK
          ...
          `;
// SQL query to get check constraint 
      ...
      tableScripts.push(createTableScript);
        ...
        // Script generation for column changes
      if (targetTable) {
...
// generate missing constraints
        if (targetTable) {
          ...
          if (missingColumns.length > 0) {
            ...
            if (!constraintsScripts.includes(alterTableScript1)) {
              constraintsScripts.push(alterTableScript1);
            }
          }
...
//console.log(existingCheckConstraints)
    ...
    const migrationScript2 = [...constraintsScripts, ...constraintScripts].join('\n');
